...
function TicketsGraficScreen() {
  ...
  const chartData = () => {
    ...
    items.forEach((ticket) => {
      if (ticket.status === "0") statusCounts.abertos += 1;
      else if (ticket.status === "1") statusCounts.encerrados += 1;
      else if (ticket.status === "2") statusCounts.improcedentes += 1;
      else if (ticket.status === "3") statusCounts.cancelados += 1;
    });
  ...
  const top5Tickets = items
    .filter((item) => item.status !== "0") // Filtra por tickets encerrados
